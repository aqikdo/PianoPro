...
# CUDA_VISIBLE_DEVICES=2,3,4,5,6 torchrun --nproc_per_node=5 multi_task/train_high_level_ddp_actioncond.py
...
def main():
    # 兼容 torchrun/launch，直接读取环境变量
    local_rank = int(os.environ.get("LOCAL_RANK", 0))
    # Set OMP_NUM_THREADS to 12 as requested
    ...
    dataloader = torch.utils.data.DataLoader(
        ...
        sampler=sampler,
        num_workers=7,  # Set to 12 as requested
        pin_memory=True,
        drop_last=True,
    ...
    noise_pred_net = ConditionalUnet1DWithActionCond(
        ...
        action_cond_dim=10,  # 新增action condition维度
        midi_dim=obs_dim,
    ...
    # DDP 封装
    noise_pred_net = DDP(noise_pred_net, device_ids=[local_rank], output_device=local_rank, find_unused_parameters=True)
    ...
    dist.barrier()  # 等待rank 0完成目录创建
    run_name = "DF-HL-ActionCond-DDP"
    ...
    for epoch_idx in epoch_pbar:
        ...
        for nbatch in batch_pbar:
            ...
            # 提取naction的后10维作为action condition
            action_cond = naction[:, :, -10:].mean(dim=1)  # (B,10)
            noise_pred = noise_pred_net(
                noisy_actions, timesteps, global_cond=obs_cond, action_cond=action_cond)
        ...
        if dist.get_rank() == 0:
            ...
            if epoch_idx % 100 == 0:
                ...
                save_dir = os.path.join(workspace_dir, f"exp/HL_DDP_AC{time.strftime('%Y%m%d_%H%M%S')}")
                os.makedirs(save_dir, exist_ok=True)
